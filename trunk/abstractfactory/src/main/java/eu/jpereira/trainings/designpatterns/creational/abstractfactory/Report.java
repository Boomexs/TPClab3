...
public class Report {
	...
	public Report(String string) {
		...
		if ( reportType.equals("JSON")) {
			reportFactory = new JSONReportFactory();
		} else {
			reportFactory = new XMLReportFactory();
		}
		this.setFooter(reportFactory.makeFooter());
		this.setHeader(reportFactory.makeHeader());
		this.setBody(reportFactory.makeBody());
	}
